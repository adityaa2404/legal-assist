...
# Prevent repeated wake-request emails from being sent too frequently.
...
async def _check_worker() -> bool:
    ...
    try:
        ...
        logger.warning(
            "Worker health check failed: url=%s error=%s",
            settings.WORKER_URL,
            exc,
        )
        return False
...
async def get_worker_status(force: bool = False) -> bool:
    ...
    if (
        not force
        and (now - _worker_status_cache["checked_at"])
        < _WORKER_STATUS_TTL_SECONDS
    ):
...
async def request_worker_wake(payload: WakeRequest):
    """
    Email the owner when a visitor's worker is taking too long to start.
    The visitor's message is optional.
    """
    ...
    remaining = _WAKE_REQUEST_COOLDOWN_SECONDS - (
        now - _last_wake_request_at
    )
    ...
    message_html = (
        ...
        if safe_message
        else "<em>No message was provided.</em>"
    )
    ...
    email_payload = {
        ...
        "html": (
            ...
            "<p>A visitor reported that the analysis worker is taking "
            "longer than expected to start.</p>"
    ...
    try:
        ...
        logger.error(
            "Resend rejected wake request: status=%s body=%s",
    ...
    return {
        "message": "Your request was sent. We will check the worker shortly."
    }
