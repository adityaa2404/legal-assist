...
def _provider_from_call(
    ...
    try:
        bound = inspect.signature(fn).bind(
            self_obj,
            *args,
            **kwargs,
        )
...
def _install_async_llm_method(
    ...
    if (
        ...
        or getattr(
            original,
            "_legal_assist_observability_wrapped",
            False,
        )
    ):
        return
    ...
    async def wrapped(
        ...
        **kwargs,
        ...
        provider_token = _CURRENT_PROVIDER.set(
            provider
        )

        purpose_token = _CURRENT_PURPOSE.set(
            purpose
        )

        fallback_token = _FALLBACK_USED.set(
            False
        )

        suppress_token = _LOW_LEVEL_SUPPRESSED.set(
            provider != "gemini"
        )
        ...
        try:
            ...
            _LOW_LEVEL_SUPPRESSED.reset(
                suppress_token
            )

            _FALLBACK_USED.reset(
                fallback_token
            )

            _CURRENT_PURPOSE.reset(
                purpose_token
            )

            _CURRENT_PROVIDER.reset(
                provider_token
            )
...
def _install_ocr_metrics() -> None:
    ...
    for method_name, mode in (
        (
            "_extract_pdf_gemini_vision",
            "fast",
        ),
        (
            "_extract_pdf_scanned",
            "secure",
        ),
    ):
        original = getattr(
...
def _install_pii_metrics() -> None:
    ...
    def wrapped(
        ...
        **kwargs,
        ...
        anonymized_text, mapping = original(
            self,
            *args,
            **kwargs,
        )
    ...
    async def wrapped(
        ...
        **kwargs,
        ...
        try:
            ...
            htoc_build_duration_seconds.labels(
                provider=provider,
            ).observe(
                time.perf_counter() - start
            )
    ...
    def wrapped(
        ...
        **kwargs,
        ...
        try:
            ...
            bm25_query_duration_seconds.observe(
                time.perf_counter() - start
            )
...
def install_service_instrumentation(
    force: bool = False,
) -> None:
    """
    Install all custom metrics around existing
    service execution paths.

    force=True is used in Celery child processes so
    each forked worker process gets the instrumentation
    even if the parent process has already initialized it.
    """
    ...
    try:
        ...
        logger.info(
            "service observability instrumentation installed"
        )

    except Exception:
        logger.exception(
            "Failed to install service observability instrumentation"
        )

        # Observability must never prevent
        # API/worker startup.
...
def _install_worker_observability(
    **kwargs,
):
    """
    Install instrumentation when the Celery worker
    is ready.

    This covers the worker process lifecycle where
    applicable.
    """
...
def _install_worker_process_observability(
    **kwargs,
):
    """
    Install instrumentation inside each Celery
    prefork child process.

    This is important because document processing
    runs inside ForkPoolWorker-* processes.
    """
